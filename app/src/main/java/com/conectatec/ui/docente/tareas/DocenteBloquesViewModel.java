...
@HiltViewModel
public class DocenteBloquesViewModel extends ViewModel {

    private final TareasRepository repository;
    private final MutableLiveData<UiState<List<Bloque>>> state = new MutableLiveData<>();
    private static final Executor BG = Executors.newSingleThreadExecutor();

    @Inject
    public DocenteBloquesViewModel(TareasRepository repository) {
        this.repository = repository;
    }

    public LiveData<UiState<List<Bloque>>> getState() { return state; }

    public void cargarBloques(int grupoId) {
        if (state.getValue() instanceof UiState.Loading) return;
        state.setValue(UiState.Loading.get());
        BG.execute(() -> {
            try {
                List<Bloque> bloques = repository.getBloques(grupoId);
                state.postValue(new UiState.Success<>(bloques));
            } catch (Exception e) {
                state.postValue(new UiState.Error<>(e.getMessage() != null
                        ? e.getMessage() : "Error al cargar bloques"));
            }
        });
    }
}
